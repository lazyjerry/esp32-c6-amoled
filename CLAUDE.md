...
[src/main.c](src/main.c) 是目前的主程式。快速上下甩板子擲一次筊，畫面是**由上往下看的第一人稱**：兩片紅木筊被拋出、落地彈跳，鏡頭再拉近看筊象，同時播木頭撞擊聲。

| 功能 | 作法 |
|------|------|
| 觸發 | QMI8658 加速度計，[imu.c](src/imu.c)。先低通估出重力方向，再把動態加速度投影到那條軸上數半程，板子怎麼拿都算得出上下 |
| 筊象 | [cast.c](src/cast.c)。聖筊 50%、笑筊 25%、陰筊 25%，立筊十萬分之一（0.001%），走 `esp_random` |
| 動畫 | [cast_ui.c](src/cast_ui.c)。俯視沒有地平線，景深靠「高度→放大＋上移」＋「影子留在地面」＋「翻滾時兩張圖交替」 |
| 音效 | [audio.c](src/audio.c)。ES8311 走 `esp_codec_dev`，落地與兩次彈跳各一聲，音量遞減 |
| **BOOT 鍵短按** | 切 light sleep（螢幕關／喚醒）。喚醒源只能是它，見筆記 |
| **PWR 鍵短按** | 不用搖，直接擲一次（測試用）。走 AXP2101 `INTSTS2` bit3 輪詢，不佔 GPIO |
