...
摘要: 板上只有 BOOT 與 PWR 兩顆鍵（無 RESET）。C6 的 LP IO 只有 GPIO0~7 且被 LCD QSPI／SD／I2C 佔滿，BOOT 在 GPIO9 只能喚醒 light sleep；PWR 直通 AXP2101 PWRKEY，長按硬體斷電攔不到，短按可由 I2C 讀 INTSTS2 bit3 得知。
狀態: 生效
建立日期: 2026-08-09
---
...
要做按鍵休眠就走 **light sleep**，喚醒源只能是 BOOT（GPIO9）：`gpio_wakeup_enable(9, GPIO_INTR_LOW_LEVEL)` + `esp_sleep_enable_gpio_wakeup()`。**不要規劃 deep sleep 靠按鍵喚醒**，這片板子做不到，deep sleep 只剩 RTC timer 一種醒法。

另一顆 PWR 鍵不接 SoC，走 AXP2101（I2C `0x34`）：

- **長按**由 PMIC 硬體斷電，韌體既攔不到也擋不掉，這是唯一真正的「關機」
- **短按**在 `INTSTS2`(`0x49`) bit3 留旗標，輪詢讀到即可當自訂按鍵，寫 1 清除；要先在 `INTEN2`(`0x41`) bit3 開啟
- 開機過程本身會留下旗標，初始化時先清一次，否則一啟動就會誤判成按了一下

進 light sleep 前要等按鍵放開再睡，否則低電位會立刻把自己叫醒。醒來後同樣要等放開，不然會被判定成第二次切換。
